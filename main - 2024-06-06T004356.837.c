...
Node* create_graph(const char *filename, int *width, int *height){
  ...
  for(unsigned j = 0; j < *height; ++j) {
    for (unsigned i = 0; i < *width; ++i) {
      ...
      node->b = pixel[2];
      node->w = pixel[3];
      node->up = j > 0 ? &nodes[(j - 1) * *width + i] : NULL;
      node->down = j < *height - 1 ? &nodes[(j + 1) * *width + i] : NULL;
      node->lt = i > 0 ? &nodes[j * *width + (i - 1)] : NULL;
      node->rt = i < *width - 1 ? &nodes[j * *width + (i + 1)] : NULL;
      node->p = node;
      node->rank = 0;
    }
  }
...
void apply_filter(unsigned char *image, int width, int height) {
  ...
  for(j = 1; j < height - 1; j++) {
    for(i = 1; i < width - 1; i++) {
      ...
      int grad_len = sqrt(sumX * sumX + sumY * sumY);
      if(grad_len > 255) 
        grad_len = 255;
      int res = (j * width + i) * 4; 
  ...
  for(j = 0; j < height; j++) {
    for(i = 0; i < width; i++) {
      Node* node = &nodes[j * width + i];
        ...
        if(node->lt) 
          union_set(node, node->lt, epsilon);
        if(node->rt) 
          union_set(node, node->rt, epsilon);
    }
  }
  return;
}
void flood(unsigned char* image, int x, int y, int new_color1, int new_color2, int new_color3, int old_color, int width, int height) {
  int dx[] = {-1, 0, 1, 0};
  int dy[] = {0, 1, 0, -1};
...
void color_components(unsigned char* image, int width, int height, int epsilon) {
  int color1 = epsilon * 5;
  int color2 = epsilon * 5;
  int color3 = epsilon * 5;
  int i, j;
  for(j = 1; j < height - 1; j++) {
    for(i = 1; i < width - 1; i++) {
      if(image[4 * (j * width + i)] < epsilon) 
        flood(image, i, j, color1, color2, color3, epsilon, width, height);
      color1 = rand() % (255 - epsilon * 5) + epsilon * 5;
      color2 = rand() % (255 - epsilon * 5) + epsilon * 5;
      color3 = rand() % (255 - epsilon * 5) + epsilon * 5;
    }
  }
}
int main() {
